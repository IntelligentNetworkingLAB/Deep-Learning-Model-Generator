...
class Conv2dLSQ(nn.Conv2d):
    ...
    def forward(self, x):
        ...
        if self.bit == 32:
            ...
            return F.conv2d(self.quan_a(x), self.quan_w(self.weight), self.bias, self.stride, self.padding, self.dilation, self.groups)*self.quan_a.get_alpha()*self.quan_w.get_alpha()
