...
import math
from base._quan_base import _Conv2dQ, Qmodes, _LinearQ, _ActQ
...
class Conv2dLSQ(_Conv2dQ):
    ...
    def forward(self, x):
        ...
        self.w_q = FunLSQ.apply(self.weight, self.alpha, g, Qn, Qp)
        # wq = y.transpose(0, 1).reshape(self.weight.shape).detach() + self.weight - self.weight.detach()
        return F.conv2d(x, self.w_q, self.bias, self.stride,
                        self.padding, self.dilation, self.groups)
