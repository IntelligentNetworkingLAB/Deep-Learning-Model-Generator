...
class FuseConv2dQ(nn.Conv2d):
    ...
    def forward(self, x):
        ...
        #f_weight, f_bias = fusing(self.weight, self.bn.weight, self.bias, self.bn.bias, u, v)
        ...
        if self.is_first_conv == True : 
            ...
            f_bias = f_bias/alpha
            out = F.conv2d(x, w_q, f_bias, self.stride, self.padding, self.dilation, self.groups) * alpha
            ...
            f_bias = f_bias/(alpha_a*alpha_w)
            
            out = F.conv2d(a_q, w_q, f_bias, self.stride, self.padding, self.dilation, self.groups) * alpha_a * alpha_w
        print(self.bn.bias)    
        return out
